...
namespace QuantConnect.Orders.Fees
{
    ...
    /// </summary>
    public class SaxoFeeModel : FeeModel
    {
        ...
        public SaxoFeeModel(decimal monthlyForexTradeAmountInUSDollars = 0, decimal monthlyOptionsTradeAmountInContracts = 0)
        {
        ...
        private static CashAmount EUREXFutureFees(Security security)
        {
            ...
            decimal saxoFeePerContract, exchangeFeePerContract;
            string symbol;
            ...
            if (!fees.TryGetValue(symbol, out saxoFeePerContract))
            {
                saxoFeePerContract = 3.00m;
            }

            if (!exchangeFees.TryGetValue(symbol, out exchangeFeePerContract))
            {
                exchangeFeePerContract = 0.50m;
            }

            var currency = Currencies.EUR;

            if (security.SymbolProperties.QuoteCurrency == "CHF")
                currency = Currencies.CHF;

            // Add exchange fees
            return new CashAmount(saxoFeePerContract + exchangeFeePerContract, currency);
        }
