...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class FuturesMomentumAlgorithmSaxo : QCAlgorithm
    {
        ...
        {
            SetStartDate(2017, 12, 18);
            SetEndDate(2017, 12, 19);
            SetAccountCurrency("CHF", 1000000);
            //SetWarmUp(Math.Max(_fastPeriod, _slowPeriod));

            // Adds the future that will be traded and
            // set our expiry filter for this futures chain

            var future = AddFuture(Futures.Indices.SMI, Resolution.Tick);
            ...
            _fast = EMA(future.Symbol, _fastPeriod, Resolution.Tick);
            _slow = EMA(future.Symbol, _slowPeriod, Resolution.Tick);
        }
